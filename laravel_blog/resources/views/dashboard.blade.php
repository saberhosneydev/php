    ...
    <div class="py-12">
        ...
        <div class="max-w-xl px-10 my-4 py-6 bg-white rounded-lg shadow-md mx-auto">
            <div class="flex justify-between items-center">
                ...
                <a class="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500" href="#">{{strtoupper($article->category->name)}}</a>
            </div>
            <div class="mt-2">
                <a class="text-2xl text-gray-700 font-bold hover:text-gray-600" href="{{ $article->category->name }}/{{ $article->slug }}">{{$article->title}}</a>
                <p class="mt-2 text-gray-600">{{Str::of($article->summary)->limit(300, ' (...)')}}</p>
            </div>
            <div class="flex justify-between items-center mt-4">
                <a class="text-blue-600 hover:underline" href="{{ $article->category->name }}/{{ $article->slug }}">Read more</a>
                <div>
